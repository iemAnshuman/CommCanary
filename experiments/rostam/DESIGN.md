...
| Exact clean CommCanary commit, built wheel filename/SHA-256, source archive hash, and dirty-patch hash if applicable | `commcanary_wheel` binds the Rostam-built, gate-verified wheel (commit, filename, SHA-256); its member content digest must match the recorded cross-platform build before use |
