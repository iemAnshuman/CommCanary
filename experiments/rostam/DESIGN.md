...
| Exact clean CommCanary commit, built wheel filename/SHA-256, source archive hash, and dirty-patch hash if applicable | `commcanary_wheel` binds the reproducible local build (commit, filename, SHA-256); the Rostam-side rebuild must reproduce that hash before use |
