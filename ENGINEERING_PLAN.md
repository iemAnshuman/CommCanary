...
release boundary. The Rostam environment, PARAM patch, wheel, venv, site, GEMM
calibration, and shared-trace prerequisites have been resolved and bound at
repository identity `2855275288e67a1a2d0bbefff0740841fdf0ecf0`.

**Physical evidence collection is finished.** Three publication-grade campaigns
totalling 280 cells are complete at one repository identity, and the trusted
join over all three regenerates byte-for-byte. The remaining work is
documentation, the release decision, and the compatibility-contract follow-up
described at the end of this checkpoint.

Two publication-grade physical campaigns at that identity are complete:

- `shared-replay-20260720-r2`: 40/40 selected successful cells, zero
  completeness issues, manifest
  ...
  and verified raw archive
  `3451ee540b634e1daad0aa49b6b95173fd601f5b6baea04deb6395d8d2c7b273`.
- `overlap-20260724-r1`: 80/80 selected successful cells, zero completeness
  issues, manifest
...
The identity-aligned replacement core campaign is also complete:

- `core-20260724-r7`: 160/160 selected successful cells, zero completeness
  issues, 640 verified artifacts totalling 4,921,885,442 bytes, manifest
...
The trusted core/shared/overlap join covers all 280 selected cells, reports
`supported-by-complete-selected-evidence`, and regenerated byte-for-byte:
...
Producing it required two reviewed analyzer repairs, both covered by tests and
neither weakening an evidence check:

1. `_validate_trusted_join` compared whole `campaign.policy` documents, but
   `catalog_profile` and `input_paths` differ by construction between profiles.
   The guard now compares the analysis-relevant policy subset; input identity
   is still enforced by `(sha256, size_bytes)` per input id.
2. Serializing the 280-cell joined aggregate exceeded the 1,000,000-item
   default. The measured document is 1,013,696 items in 8,101,263 bytes, so the
   publication serializer now takes an explicit 4,000,000-item budget. This
   bounds output the pipeline just built from validated evidence; every reader
   of untrusted input keeps its original budget.

What the joined evidence supports, stated against `W-full` over 28
configuration pairs: shared-trace overlap replay agrees on 71.4% of pairs
