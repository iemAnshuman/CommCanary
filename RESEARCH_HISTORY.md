# CommCanary research history and detailed contracts

> This document preserves the longer research narrative, exact-replay
> workflow, qualification contracts, simulator examples, and implementation
> detail that previously occupied the main README. It is supporting research
> documentation, not the current product-status boundary. Start with
> [`README.md`](README.md) and [`docs/product-status.md`](docs/product-status.md).
...
> source-verifiable research framework and an early compiler contract for
> physical distributed-performance tests. It is not yet a validated
> performance gate.

**Product target.** Turn an expensive distributed-AI workload and a regression
policy into a short physical test that decides whether a stack change should
ship.

**Current study boundary.** Single-node, four-GPU tensor-parallel dense-decoder
inference with GEMM and all-reduce dependency structure. vLLM is the first
study and SGLang is the planned replication. Multi-node communication,
expert-parallel all-to-all, CUDA graphs, multi-stream execution, KV-cache
pressure, and network congestion are not qualified.
...
commcanary build trace.et --projection trace.projection.json \
  --policy regression-policy.json --oracle-corpus study/oracle-corpus.json \
...
The child process in `capture` must emit complete all-rank collective and GEMM
recipe evidence. The physical-study workflow produces the measured inputs to
`build`; they are not bundled examples. Without a real corpus, `build` emits a
blocked bundle and exits non-zero. See the
[operator quick start](docs/operator-quickstart.md) for that safe local
orientation path.
...
No real application corpus or held-out reduced-canary result is checked in.
Synthetic fixture corpora cannot qualify a bundle. The complete reader-facing
boundary is in [Product status](docs/product-status.md); the canonical
machine-readable boundary is [`claims/public-claims.yaml`](claims/public-claims.yaml).
...
- [Research history and current checkpoint](HANDOFF.md)
...
application runs preserve all of that but need model code, data, and a cluster.
The intended middle ground is a workload-shaped physical unit test: short
enough for CI, specific to a declared regression policy, and backed by
application measurements rather than a simulator alone.
...
That gate replayed the complete trace-derived event program; its `source` wire
key was not the original application. The proposed replicated follow-up binds
the reference and materialization labels to the same compiled instruction
path, so the large follow-up profile is retired rather than submitted. Exact
materialization remains a small conformance and measurement-floor control.
...
CommCanary treats two physical products separately. The exact qualification
capsule reconstructs the full trace-derived program. The new physical-canary
compiler emits fewer dependency-closed Chakra nodes, but it earns a claim only
from measured application decisions and an untouched holdout. No real
...
## Physical canary workflow details

The three commands above are the product surface. `build` refuses unsupported
operations, non-closed dependency regions,
forged content identities, incomplete candidate measurements, holdout policy
failures, and privacy-policy failures. It can emit a blocked measurement
candidate when the corpus is absent, but that command exits 1 and the bundle
cannot be used as a comparable gate. `gate` re-verifies the complete bundle
before reading observations. The policy and corpus bind a declared OCI runner
digest. Active-study audit bundles retain the raw application allocations and
physical candidate measurements, and verification recomputes their corpus
rows. The Rostam study separately binds and executes an exact OCI/SIF runner;
`commcanary build` itself does not fetch images.

The capture command projects complete instrumented shards or supported Kineto
profiles; it does not infer missing all-rank arrivals or GEMM recipes from an
arbitrary opaque process. The full format and assurance contract is in
[`docs/physical-canary.md`](docs/physical-canary.md).
...
## Ecosystem interop: Chakra carrier, Kineto import, and legacy PARAM export

Chakra already provides the ecosystem's general workload graph: its
[official schema](https://github.com/mlcommons/chakra) represents compute,
memory, communication, dependencies, timing, and resource constraints.
CommCanary should not replace that interchange layer.
Its intended boundary is the smaller one Chakra does not supply: reduce a
shared workload description to a policy-specific physical acceptance case and
retain the evidence used to select it.

`commcanary build` now consumes Chakra's official length-delimited protobuf
framing. The reader validates node IDs, types, control and data dependencies,
and graph acyclicity while retaining each complete protobuf message
byte-for-byte. It independently checks standard `comm_type` and `comm_size`
attributes for projected all-reduces; other attributes are not interpreted or
rewritten. An exact source-bound projection supplies the narrow workload
semantics and disclosure review that cannot be inferred safely from opaque
attributes. Reduced output remains a Chakra ET, so CommCanary does not create
a parallel workload graph.

This is carrier-level interoperability, not proof that every Chakra producer's
attribute vocabulary is understood. `capture` and `import-kineto` can emit the
ET and projection directly when the source evidence is complete. Arbitrary
uninstrumented `torchrun` capture remains unsupported. The Rostam product study
uses a digest-pinned Chakra runner; legacy PARAM remains confined to the exact
replay research path.
...
Chakra's graph semantics. Successful Chakra subgraph construction alone does
not claim physical execution. The product-study runner and its retained
measurements supply that separate boundary.
...
record_collective(
    ...
    rank_arrival_us={str(rank): rank * 2.5 for rank in range(4)},
...
General trace capture may omit the source-bound reduction and compute recipe.
Physical projection does not guess them: a reduction collective requires
`reduction_op`, and every projected region requires a shared `compute_recipe`
or a complete `compute_recipe_by_rank` mapping.
...
Add `--chakra-output trace.et --projection-output trace.projection.json` to
write the physical carrier and source-bound projection in the same run.
...
- `commcanary.chakra_projection.v1`
- `commcanary.physical_canary_policy.v1`
- `commcanary.physical_oracle_corpus.v1`
- `commcanary.application_measurement.v1`
- `commcanary.application_oracle.v1`
- `commcanary.application_evidence_set.v1`
- `commcanary.physical_execution_measurement.v1`
- `commcanary.physical_execution_evidence_set.v1`
- `commcanary.active_physical_study_ledger.v1`
- `physical_decision_canary.v1`
- `commcanary.physical_gate_observation.v1`
- `commcanary.physical_gate_result.v1`
