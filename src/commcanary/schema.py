...
def validate_comparison(comparison: Mapping[str, Any]) -> None:
    ...
    expected_evaluations = comparison_policy_evaluations(comparison)
    ...
    derived_verdict = derive_comparison_verdict(comparison)
...
def comparison_policy_evaluations(comparison: Mapping[str, Any]) -> List[JsonDict]:
    thresholds = comparison.get("thresholds")
    if not isinstance(thresholds, Mapping):
        raise SchemaError("comparison thresholds must be an object")
    baseline_metrics = _comparison_metrics(comparison, "baseline")
        ...
        for row in rows:
            ...
            for metric in ("median", "p95", "p99"):
                ...
                if abs(as_float(row.get(f"{metric}_absolute_us")) - expected_abs) > 0.001:
                    raise SchemaError(f"comparison breakdown_delta.{key}.{metric}_absolute_us does not match row metrics")
...
def derive_comparison_verdict(comparison: Mapping[str, Any]) -> str:
    evaluations = comparison_policy_evaluations(comparison)
