...
from .policy import ComparisonThresholdPolicy
...
def comparison_reason_codes(
    ...
    *,
    ...
    """Return ordered non-pass evaluation codes without changing the v2 wire."""
    ...
    if not isinstance(evaluations, list):
        raise SchemaError("comparison evaluations must be a list")
    codes: List[str] = []
    for index, evaluation in enumerate(evaluations):
        if not isinstance(evaluation, Mapping):
            raise SchemaError(f"comparison evaluation {index} must be an object")
        result = evaluation.get("threshold_result")
        code = evaluation.get("metric")
        if result not in {"pass", "warn", "fail"}:
            raise SchemaError(f"comparison evaluation {index} has an invalid threshold_result")
        if not isinstance(code, str) or not code:
            raise SchemaError(f"comparison evaluation {index} metric must be a non-empty string")
        if result != "pass":
            codes.append(code)
    return tuple(codes)
...
def compare_reports(
    ...
    *,
    ...
    threshold_policy: Optional[ComparisonThresholdPolicy] = None,
    limits: ResourceLimits = DEFAULT_RESOURCE_LIMITS,
) -> JsonDict:
    validate_report(baseline, limits=limits)
    validate_report(candidate, limits=limits)
    if threshold_policy is None:
        threshold_policy = ComparisonThresholdPolicy.from_legacy_arguments(
    ...
    elif not isinstance(threshold_policy, ComparisonThresholdPolicy):
        raise SchemaError("threshold_policy must be a ComparisonThresholdPolicy")
    p99_threshold_pct = threshold_policy.p99_threshold_pct
    p95_threshold_pct = threshold_policy.p95_threshold_pct
    median_threshold_pct = threshold_policy.median_threshold_pct
    p99_absolute_threshold_us = threshold_policy.p99_absolute_threshold_us
    p95_absolute_threshold_us = threshold_policy.p95_absolute_threshold_us
    median_absolute_threshold_us = threshold_policy.median_absolute_threshold_us
    hidden_drop_threshold_points = threshold_policy.hidden_drop_threshold_points
    breakdown_threshold_pct = threshold_policy.breakdown_threshold_pct
    breakdown_absolute_threshold_us = threshold_policy.breakdown_absolute_threshold_us
    ...
    breakdown_failures = _breakdown_regression_reasons(
        "phase", phase_deltas, breakdown_threshold_pct, breakdown_absolute_threshold_us
    ) + _breakdown_regression_reasons("operation", op_deltas, breakdown_threshold_pct, breakdown_absolute_threshold_us)
    ...
    if verdict == "pass":
        breakdown_warnings = _breakdown_regression_reasons(
    ...
    comparison: JsonDict = {
        ...
        "verdict": verdict,
        "reasons": reasons,
        "thresholds": threshold_policy.to_wire(),
    ...
    comparison["evaluations"] = comparison_policy_evaluations(comparison, limits=limits)
    comparison["derived_verdict"] = derive_comparison_verdict(comparison, limits=limits)
    comparison["verdict"] = comparison["derived_verdict"]
    validate_comparison(comparison, limits=limits)
    return comparison


def _breakdown_deltas(base_rows: Any, candidate_rows: Any) -> List[JsonDict]:
    base = {
        str(row.get("name")): row for row in base_rows if isinstance(row, Mapping) and isinstance(row.get("name"), str)
    }
    candidate = {
...
def _regression_reason(
    ...
    if delta_pct is None:
        return (
            f"{label} regression is new nonzero latency ({candidate - base:.3f} us absolute delta from a zero baseline)"
        )
