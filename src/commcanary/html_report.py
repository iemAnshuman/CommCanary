...
def _histogram(values: Iterable[float], buckets: int = 28) -> str:
    ...
    for index, count in enumerate(counts):
        ...
        bars.append(f'<rect x="{x:.2f}" y="{y:.2f}" width="{bar_width:.2f}" height="{bar_height:.2f}" rx="2" fill="{color}"/>')
    return (
...
def _breakdown_table(rows: Iterable[Mapping[str, Any]]) -> str:
    ...
    return "<table><thead><tr><th>Name</th><th>Count</th><th>Median us</th><th>P99 us</th></tr></thead><tbody>" + "".join(body) + "</tbody></table>"
...
def _comparison_table(rows: Iterable[tuple]) -> str:
    ...
    for label, baseline, candidate in rows:
        body.append(
            "<tr>"
            f"<td>{_esc(label)}</td>"
            f"<td>{as_float(baseline):.2f}</td>"
            f"<td>{as_float(candidate):.2f}</td>"
            "</tr>"
        )
    return "<table><thead><tr><th>Metric</th><th>Baseline</th><th>Candidate</th></tr></thead><tbody>" + "".join(body) + "</tbody></table>"
...
def _write(path: str, content: str) -> None:
    ...
    try:
        ...
        raise SchemaError(f"cannot write HTML report {path}: {exc}") from exc
